...
3. **Early Detection Imperative:** These results provide strong evidence-based justification for screening mammography—detecting tumors at T1 rather than T4 could substantially improve survival outcomes.
...
3. **Treatment Implications:** Nodal status guides treatment decisions—patients with N2-N3 disease typically receive more aggressive adjuvant chemotherapy and radiation to address the higher recurrence risk.
...
if(global_p > 0.05) {
  cat("✓ Proportional hazards assumption now satisfied.\n")
}
...
## Logistic Regression & Fairness Analysis
...
cat("Data Split:\n")
cat("• Training:", nrow(train), "patients (70%)\n")
cat("• Testing:", nrow(test), "patients (30%)\n")
...
cat("• AUC-ROC:", round(as.numeric(auc_val), 3), "\n")
```
...
<p class="casefile-lede">
<strong>Count 1 - Causing Mortality:</strong> GUILTY — `r deaths` deaths (`r fatality_rate`% fatality rate)
</p>

<p class="casefile-lede">
<strong>Count 2 - Disparate Impact:</strong> GUILTY — Black patients experience 1.7x higher mortality
</p>

<p class="casefile-lede">
<strong>Count 3 - Algorithmic Bias:</strong> NOT GUILTY — Models perform equitably across racial groups
</p>
